...
| **Context collapse** | ✅ | In-run + persistent history: `search_tools` вырезается, `use_tool` сохраняется |
...
**Persistent history** (между сообщениями) хранит:
- `user` — твой запрос
- `assistant` + `tool` — пары `use_tool` с полным JSON-ответом
- `assistant` — финальный текст ответа

`search_tools` и supervisor injections **не сохраняются**.

Подробнее — раздел **Context Collapse** ниже.
...
Context collapse = **чистый контекст в RAM** между turn'ами.  
Отдельная задача (ещё не сделано): history **переживает рестарт** бота (SQLite/Redis) — см. «Известные проблемы §3».
...
| `CHAT_MAX_HISTORY` | `20` | Turn'ов в памяти (in-memory) |
...
### 3. History не переживает рестарт

Context collapse ✅ — контекст между turn'ами чистый (см. раздел выше).  
Но history только в RAM — **теряется при рестарте** бота. Нет SQLite/Redis persistence.
...
*Последнее обновление: **3 июля 2026, 19:00** (UTC+2)*
