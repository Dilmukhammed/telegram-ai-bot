...
def attachment_maintenance_source_ref(*, user_id: int) -> str:
    return f"user:{user_id}:attachment"


def ensure_attachment_maintenance_source_version(
    service: "MemoryService",
    *,
    user_id: int,
) -> str:
    source_ref = attachment_maintenance_source_ref(user_id=user_id)
    ...
    pointer = EvidencePointer(
        ...
        location={"workspace_path": f".memory/maintenance/{user_id}/attachment"},
    )
    result = service.register_source(
        SourceInput(
            ...
            content_hash=f"attachment-maintenance:{user_id}",
            occurred_at=now,
            pointer=pointer,
        )
    )
