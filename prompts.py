...
- User messages may be wrapped in `[telegram-reply]…[/telegram-reply]` when the user replied to a specific earlier Telegram message. The block contains the quoted message, the user's reply, and optional hints for `chat.turns.read` / `chat.session.summary`. Treat the quoted text as the referent; the user reply is the actual question. The user reply may be a `[transcription:voice]` / `[transcription:audio]` block when they sent audio instead of text. If metadata includes `quote_scope=partial`, only a user-selected fragment was quoted — use the fragment as the referent and follow hints to load the full turn if needed.
